...
{
    public class LocalRenderStream : MonoBehaviour
    {
...
#pragma warning restore 0649
        ...
        IEnumerator GetOffer()
        {
            ...
            foreach (var offer in obj.offers)
            {
                ...
                Debug.Log($"Local - Received offer: {offer.sdp}");
                ...
                {
                    Debug.Log($"Local - Adding video track for connection: {connectionId}");
                    pc.AddTrack(track);
                }
                foreach(var track in audioStream.GetTracks())
                {
                    Debug.Log("Local - Adding audio track");
                    pc.AddTrack(track);
                }
        ...
        {
            var op = signaling.GetAnswer(sessionId, lastTimeGetAnswerRequest);
            yield return op;
            ...
            foreach (var answer in obj.answers)
            {
                ...
                if (!pcs.TryGetValue(answer.connectionId, out pc))
                {
                    Debug.LogError($"Network Error: Failed to find peer for answer");
                    continue;
                }
                ...
                Debug.Log($"Local answer: {answer.sdp}");
        ...
        IEnumerator Answer(string connectionId)
        {
            ...
            Debug.Log($"Local posting answer: {op.desc.sdp}");
